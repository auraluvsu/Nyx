...
func BestMove(board [8][8]*nyx.Piece, depth int, turn nyx.Colour) SearchResult {
	...
	genMoves(turn, board, nil, func(fx, fy, tx, ty int, p, cap *nyx.Piece, nextEnPassant *nyx.Position) {
		board[tx][ty], board[fx][fy] = p, nil
		score := Minimax(board, depth-1, math.MinInt32, math.MaxInt32, turn, nyx.OppositeColour(turn), nextEnPassant)
